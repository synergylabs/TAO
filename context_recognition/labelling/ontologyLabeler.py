...
class ontologyLabeler:
    ...
    def label_clusters(self, context_representations):
            ...
            for activity_idx, activity_vec in enumerate(np.reshape(context_vec, (-1, num_activities))):
                ...
                if len(activity_set) > 0:
                    ...
                    for activity in activity_set:
                        activity_ontolist = contextList.activity_mapping[self.config.dataset][activity]
        ...
        for context_idx, context_activity_map in enumerate(context_activites):
            if len(context_activity_map) > 0:
                ...
                context_labels.append(f'C{context_idx}:{onto_label}')
            else:
                context_labels.append(f'C{context_idx}:None')
    ...
    def get_onto_label(self, context_activity_map):
        ...
        parallel_context_scores = {ctx: 0 for ctx in contextList.all_contexts}
        for activity_set in deduped_activity_map:
            if len(activity_set) > 1:
                for activityA, activityB in list(combinations(activity_set,2)):
                    par_context = self.get_parallel_onto_context([activityA,activityB])
                    parallel_context_scores[par_context] += 1

        # get context scores based on sequential combinations
        sequential_context_scores = {ctx: 0 for ctx in contextList.all_contexts}
        for i in range(len(deduped_activity_map) - 1):
            ...
            for activity_A in activity_set_A:
                for activity_B in activity_set_B:
                    seq_context = self.get_sequential_onto_context([activity_A,activity_B])
                    sequential_context_scores[seq_context] += 1
        ...
        overall_context_scores = {ctx: (sequential_context_scores[ctx] +
                                  ...
                                  for ctx in contextList.all_contexts}
        onto_label = ''
        ...
        for ctx in overall_context_scores.keys():
            if overall_context_scores[ctx] > 0:
                onto_label += f'{ctx}:{overall_context_scores[ctx] / sum_context_scores:.1f}'
        if onto_label == '':
            onto_label = 'undefined'
        return onto_label

    def get_sequential_onto_context(self,activity_set):
        '''
        Gets sequential context from ontology
        :param activity_set:
        :return:
        '''
        context = 'None'
    ...
    def get_parallel_onto_context(self, activity_set):
        '''
        Gets parallel context from ontology
        :param activity_set:
        :return:
        '''
        context = 'None'
