...
def parse_aruba_dataset(rawdatasrc,
    ...
    if args.mode == 'train':
        ...
        cache_file = f'{cache_dir}/aruba_stack_merge_{args.lag_parameter}_{args.merge_mins}_{args.sliding_parameter}_{args.max_time_interval_in_mins}_{args.merge_mins}.pb'
        if access_cache:
...
def parse_witham_dataset(rawdatasrc,
                         args=None,
                         cache_dir='cache/',
                         access_cache=False,
                         logger=None):
    if args.mode == 'train':
        # check if cache dir is available, and try to read data from cache
        cache_file = f'{cache_dir}/witham_stack_merge_{args.lag_parameter}_{args.merge_mins}_{args.sliding_parameter}_{args.max_time_interval_in_mins}_{args.merge_mins}.pb'
        if access_cache:
            if not os.path.exists(cache_dir):
                os.makedirs(cache_dir)
            # check if file exists
            if os.path.exists(cache_file):
                logger.info("Got Witham Data from cache..")
                X = pickle.load(open(cache_file, 'rb'))
                return X
