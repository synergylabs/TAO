...
class ontoconvdirectLabeler:
    ...
    def label_clusters(self, context_representations):
        ...
        context_activites = []
        activity_labels = np.array(self.config.onto_activity_labels)
        num_activities = len(self.config.onto_activity_labels)
        for context_idx, context_vec in enumerate(self.context_representations):
            ...
            for activity_idx, activity_vec in enumerate(np.reshape(context_vec, (-1, num_activities))):
                ...
                if len(activity_set) > 0:
                    activities.append(activity_set)
        ...
        for context_idx, context_activity_map in enumerate(context_activites):
            if len(context_activity_map) > 0:
                ...
                context_labels.append(f'C{context_idx}(unknown)__None:1.0')
    ...
    def save(self):
        exp_model_dir = f'{self.config.cache_dir}/{self.config.experiment}/models'
        ...
        labeler_file = f'{exp_model_dir}/ontoconvdirect_labels_{self.config.model_re}' \
